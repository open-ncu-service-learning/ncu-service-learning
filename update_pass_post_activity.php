...
//新的資料庫活動編號和舊的資料活動編號
	$sql = "SELECT * FROM `news_activity` WHERE `news_id`='$id'";
	...
	$begin_time = $row['news_begin_time'];
	$end_time = $row['news_end_time'];

// 資料公告更新	//更改認證類別post的來源
	$sql = "UPDATE `pass`.`news_activity`
		    SET
				`news_title`			= '$postedtitle',
				`news_location`			= '$_POST[location]',
				`news_begin_time`		= '$begin',
				`news_end_time`			= '$end',
				`news_act_type`			= '$_POST[type]',
				`news_life_sub`			= '$_POST[life_sub]',
				`news_text`				= '$des',
				`news_hour`				= '$hour',
				`news_pass_type`		= '$_POST[service_hour_type]',
				`news_sticker_number`	= '$_POST[sticker]',
				`news_semester`			= '$semester',
				`news_requirement`      = '$req',
				`news_link`				= '$_POST[link]',
				`news_file`				= '$str',
				`news_person`			= '$_POST[person]',
				`news_office`			= '$_POST[office]',
				`news_email`			= '$_POST[email]',
				`news_phone`			= '$_POST[phone]'
			WHERE
				`news_id`				= '$id'
		";
...
//資料活動更新 //更改認證類別post的來源
	$sql = "UPDATE `pass`.`activity` 
				SET
					...
					`act_sticker_number`   = '$_POST[sticker]',
					...
					`act_ip`               = '$ip'
				WHERE 
					`act_title`	= '$title' AND `act_begin_time` = '$begin_time' AND `act_end_time` = '$end_time' AND act_id ='$sub_id' ";
	...
	if($ret)
	{
		...
		"
			<script>
				alert(\"修改公告成功\");
				self.location.href='pass_activity_content.php?news_id=$id';
			</script>
		";
	}
	else
